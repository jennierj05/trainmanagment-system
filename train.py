...
def login():
    ...
    connection = con.connect(host='localhost',port='3306',user='root', password='jennie@123', database='rail1')
...
def register():
   ...
   #trial
    ...
    messagebox.showinfo("Success", "Register Succesful", parent=w1)
...
#LABELLING AND PACKING
    ...
    x=tk.Label(w1,text="TN RAIL LOGIN ", font=("SF PRO DISPLAY", 30) , fg='Blue',borderwidth=4, relief=tk.RIDGE)
...
def final():
    ...
    tk.Label(text=str,font=("CENTURY GOTHIC",20,"bold"),fg="orangered").place(x=450,y=650)
...
def booked():
    ...
    if number_of_passengers==2:
        ...
        con=my.connect(host="localhost",port='3306',user='root',password='jennie@123',db='rail1')
        cursor=con.cursor()
        ...
        import mysql.connector as my
        con=my.connect(host="localhost",user='root',password='jennie@123',db='rail')
        cursor=con.cursor()
...
def train():
    ...
    con=connection.connect(host="localhost",user="root",password="jennie@123",db="rail1")
    cursor=con.cursor()
